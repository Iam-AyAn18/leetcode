...
What are the advantages of using upper bounded and lower bounded wildcards in Java, like `<? super Animal>` and `<? extends Animal>`?
*/
...
class Member {
    ...
    public String toString() {
        return "Member ID: " + memberId + ", Name: " + name + ", Membership Status: " + membershipStatus + ", List Workout: " + listWorkout;
    }
}
...
class Membership {
    ...
    */
    ...
    public void addMember(Member member) {
        ...
        if(member.listWorkout == null) {
            member.listWorkout = new ArrayList<>();
        }
        members.add(member);
    }
    ...
    public void addWorkout(int memberId, Workout workout) {
        ...
        for (Member member : members) {
            if (member.memberId == memberId) {
                member.listWorkout.add(workout);
                break;
            }
        }
    }
    public Map<Integer, Double> getAverageWorkoutDurations() {
        ...
        {
            int totalDuration = 0;
            for(Workout workout : member.listWorkout){
                totalDuration += workout.getDuration();
            }
            result.put(member.memberId, totalDuration/(double)member.listWorkout.size());
        }
